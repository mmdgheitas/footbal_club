...
-- ترتیب صحیح ایجاد جداول برای رفع Circular Reference

-- 1️⃣ اول fc_classrooms بدون Foreign Key
CREATE TABLE IF NOT EXISTS fc_classrooms (
    id INT PRIMARY KEY AUTO_INCREMENT,
    uuid CHAR(36) NOT NULL UNIQUE,
    name VARCHAR(255) NOT NULL,
    ...
    INDEX idx_coach_id (coach_id),
    INDEX idx_created_at (created_at)
    -- ⚠️ Foreign Key به fc_users بعداً اضافه می‌شود
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS fc_alerts (
    ...
    target_age_min INT NULL,
    target_age_max INT NULL,
    created_by INT NULL,                    -- ✅ تغییر به NULL
    priority ENUM('low', 'medium', 'high', 'urgent') DEFAULT 'medium',
    expires_at TIMESTAMP NULL,
    deleted_at TIMESTAMP NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    FOREIGN KEY (created_by) REFERENCES fc_users(id) 
        ON DELETE SET NULL ON UPDATE CASCADE,   -- ✅ حالا با NULL سازگار است
